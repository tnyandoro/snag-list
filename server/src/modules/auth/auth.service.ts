...
export class AuthService {
  ...
  async register(input: RegisterInput) {
    ...
    try {
      ...
      const userResult = await client.query(
        `INSERT INTO users (id, email, password_hash, full_name, role_id, phone, is_active, email_verified_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())
         RETURNING id, email, full_name, phone, is_active, created_at`,
        [uuidv4(), input.email, password_hash, input.full_name, role_id, input.phone, true]
      );

      const user = userResult.rows[0];

      // Generate JWT token
      const token = this.generateToken(user.id, input.role.toLowerCase());
      const expires_at = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();

      await client.query('COMMIT');

      return {
        user: { ...user, role: input.role.toLowerCase() },
        token,
        expires_at,
      };
    } catch (error) {
  ...
  async login(email: string, password: string): Promise<AuthResult> {
    const result = await pool.query(
      `SELECT u.id, u.email, u.password_hash, u.full_name, u.phone, u.avatar_url, 
              u.is_active, u.email_verified_at, r.name as role
       FROM users u
  ...
  async resetPassword(token: string, password: string) {
    ...
    await pool.query(
      'UPDATE users SET password_hash = $1 WHERE email_verified_at IS NOT NULL',
      [password_hash]
    );
  }
